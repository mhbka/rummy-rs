...
pub fn create_basic_game_with_history(
    player_count: usize,
) -> Result<History<BasicRummyGame>, GameSetupError> {
    let player_ids: Vec<usize> = (0..player_count).collect();
    let deck_config = DeckConfig {
        shuffle_seed: Some(SHUFFLE_SEED),
        pack_count: 1,
...
pub fn create_basic_game_with_replay(
    player_count: usize,
    skip_failed_actions: bool,
) -> Result<Replay<BasicRummyGame>, GameSetupError> {
    let player_ids: Vec<usize> = (0..player_count).collect();
    let deck_config = DeckConfig {
        shuffle_seed: Some(SHUFFLE_SEED),
        pack_count: 1,
    ...
    History::new(player_ids, config, deck_config).map(|g| Replay::new(g, skip_failed_actions))
}
