...
fn history_is_correct() {
    ...
    let failed_discard = game.execute_action(GameAction::Discard(DiscardAction { card_index: 10000, declare_going_out: None }));
    ...
    let round_history = game
        ...
        .get(&1)
        .unwrap();
    assert_eq!(round_history[0].entry, GameInteractions::Action(GameAction::DrawDeck(DrawDeckAction {})));
    assert_eq!(round_history[0].successful, true);

    assert_eq!(round_history[1].entry, GameInteractions::HandRearrangement { player_id: rearranging_hand_player_id, new_arrangement: hand });
    assert_eq!(round_history[1].successful, true);

    assert_eq!(round_history[2].entry, GameInteractions::Action(GameAction::FormMeld(FormMeldAction { card_indices: indices })));
    assert_eq!(round_history[2].successful, true);

    assert_eq!(round_history[3].entry, GameInteractions::Action(GameAction::Discard(DiscardAction { card_index: 10000, declare_going_out: None })));
    assert_eq!(round_history[3].successful, false);

    assert_eq!(round_history[4].entry, GameInteractions::Action(GameAction::Discard(DiscardAction { card_index: 0, declare_going_out: None })));
